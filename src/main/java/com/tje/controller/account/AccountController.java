...
// GET .../accounts => AccountController -> method(Get 요청을 처리할 메서드)
...
public class AccountController {
    ...
    public List<Account> getAccountList(){
        ...
        // 전체 필드 매개변수 생성자로 생성
        // 매개변수 순서가 필드 순서와 같음
        list.add (new Account("0001","홍길동",10000,new Date().getTime()));

        // 빌더 패턴으로 만듦
        list.add(Account.builder().ownerName("김철수").ano("0002").balance(20000).createTime(new Date().getTime()).build());
